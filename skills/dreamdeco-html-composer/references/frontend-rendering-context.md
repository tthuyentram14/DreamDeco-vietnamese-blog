...
## Nguyên tắc styling: CSS custom properties

Styling chính bằng **Tailwind utility classes** — các utility này đã backed bởi CSS custom properties của Tailwind v4/`globals.css` (ví dụ `bg-sky-50` tương ứng `var(--color-sky-50)`). KHÔNG cần inline `style` fallback với hex/px cho color, radius, shadow.
...
### BẮT BUỘC
- Styling bằng Tailwind utility classes; không inline hex/px-radius/non-var shadow
